...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        ...
        while line != "":
            ...
            # Preparing dogname: striping newline from line
            dogname = line.rstrip()
            ...
            if dogname not in dognames_dic:
                ...
                print("** Warning: Duplicate dogname exist in directory: ", dogname)
    ...
    for key in results_dic:
        ...
        if results_dic[key][0] in dognames_dic:
            ...
            # Classifier Label IS image of Dog (e.g. found in dognames_dic) appends (1, 1)
            # because both labels are dogs
            if results_dic[key][1] in dognames_dic:
                results_dic[key].extend((1, 1))

            # Classifier Label IS NOT image of dog (e.g. NOT in dognames_dic) appends (1,0)
            # because only pet label is a dog
            else:
                results_dic[key].extend((1, 0))

        # Pet Image Label IS NOT a Dog image (e.g. NOT found in dognames_dic)
        else:
            # Classifier Label IS image of Dog (e.g. found in dognames_dic) appends (0, 1)
            # because only Classifier labe is a dog
            if results_dic[key][1] in dognames_dic:
                results_dic[key].extend((0, 1))

            # Classifier Label IS NOT image of Dog (e.g. NOT in dognames_dic) appends (0, 0)
            # because both labels aren't dogs
            else:

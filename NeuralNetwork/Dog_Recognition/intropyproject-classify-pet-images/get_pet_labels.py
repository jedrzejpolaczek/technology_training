...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
        ...
        # Skips file if starts with . because it isn't an pet image file
        if in_files[idx][0] != ".":

            # Creates temporary label variable to hold pet label name extracted
            pet_label = in_files[idx]

            # Preparing the label: removing extensions
            pet_label = pet_label[:pet_label.rfind(".")]

            # Preparing the label: removing digits
            pet_label = ''.join([i for i in pet_label if not i.isdigit()])

            # Preparing the label: removing '_'
            pet_label = pet_label.replace("_", " ")

            # Preparing the label: strips leading & trailing whitespace characters
            pet_label = pet_label.strip()

            # Preparing the label: make all characters a lower case characters
